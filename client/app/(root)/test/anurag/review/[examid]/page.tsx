			...
			<div className="flex justify-between z-50 p-5 fixed navbar backdrop-filter backdrop-blur-lg bg-opacity-30  bg-base-100">
				<div className="flex items-center">
					<button
						...
						onClick={() => (window.location.href = `/valuate/${valuatorId}`)}
					>
						<FiArrowLeft className="text-2xl" />
					</button>
					<div className="flex items-center">
						<p className="font-semibold text-2xl min-w-fit mr-10">{totalMarks?.examName}</p>
						<p className="mr-5 flex items-center font-semibold text-lg">
						...
						<button
							...
							onClick={() => (document.getElementById("revaluation_modal") as any).showModal()}
						>
							<FiRefreshCcw /> Revaluate
						</button>
					</div>
				</div>
				<div className="join">
					<button onClick={() => setView(0)} className={"btn join-item " + (view === 0 ? "btn-primary" : "")}>
						Answer Sheet
					</button>
					<button onClick={() => setView(1)} className={"btn join-item " + (view === 1 ? "btn-primary" : "")}>
						Question Paper
					</button>
					<button onClick={() => setView(2)} className={"btn join-item " + (view === 2 ? "btn-primary" : "")}>
						Answer Key
					</button>
				</div>
			</div>
			<div className="flex w-full h-full">
				<div className="mb-10 p-10 pt-[100px] w-full">
					<p className="font-semibold text-xl mb-5 flex items-center">
						<AiOutlineTrophy className="mr-2" /> Total marks scored: {totalMarks?.totalScore} /{" "}
						{totalMarks?.maxScore}
					</p>
					{(valuations[selectedValuation] as any)?.data?.answers?.map((item: any, index: number) => {
						return (
							<div className="flex my-3 max-w-7xl" key={index}>
				...
				<div className="pt-20 w-[60vw] h-screen mr-5">
					<img
						src={
							[
								(valuations[selectedValuation] as any)?.answerSheet,
								(valuations[selectedValuation] as any)?.questionPaper,
								(valuations[selectedValuation] as any)?.answerKey,
							][view]
						}
			...
			<dialog id="revaluation_modal" className="modal">
				<div className="modal-box max-w-2xl align-middle">
					<h3 className="flex items-center font-bold text-2xl mb-5">
						<FiRefreshCcw className="mr-2" /> Revaluate{" "}
						{(valuations[selectedValuation] as any)?.data?.student_name}&apos;s answer sheet
					</h3>
					<p className="mb-5 font-semibold">Remarks</p>
					<textarea
						className="textarea textarea-bordered w-full"
						placeholder="Remarks"
						value={revaluationRemarks}
						onChange={(e) => setRevaluationRemarks(e.target.value)}
					></textarea>
					<button
						className={"mt-10 btn w-full btn-primary " + (revaluating ? "opacity-60" : "")}
						onClick={() => {
